...
async fn apply_passive_reload(state: &ferrum_edge::proxy::ProxyState, incremental: bool) {
    ...
    let filler = next.proxies.iter_mut().find(|proxy| proxy.id == "filler").unwrap();
    ...
    let outcome = if incremental {
        state.apply_incremental(IncrementalResult {
...
async fn passive_discovery_ejection_survives_reload(incremental: bool) {
    ...
    let mut static_upstream = upstream_with_sd(
        "reload-static", vec![bad_target.clone(), good_target.clone()], None,
    );
    for upstream in [&mut discovered_upstream, &mut static_upstream] {
        upstream.health_checks = Some(serde_json::from_value(serde_json::json!({
            "passive": passive
        })).unwrap());
    }
    let mut config = config_with(vec![discovered_upstream, static_upstream]);
    for (id, upstream_id) in [
        ("discovered", "reload-discovered"), ("static", "reload-static"),
        ("partial", "reload-discovered"), ("filler", "reload-static"),
    ] {
        config.proxies.push(serde_json::from_value(serde_json::json!({
    ...
    let (state, handles) = ProxyState::new(
        config, DnsCache::new(Default::default()),
        ferrum_edge::config::EnvConfig { pool_warmup_enabled: false, ..Default::default() },
        None, None,
    ).unwrap();
    ...
    let control = ferrum_edge::_test_support::apply_service_discovery_snapshot_for_test(
        "ferrum", "reload-discovered", "scripted", publish(initial), &mut discovery,
        &state.load_balancer_cache, &request_epoch, &[], LoadBalancerAlgorithm::RoundRobin,
        &None, &cancel_rx, &None, &state.dns_cache, &state.health_checker,
        &state.circuit_breaker_cache,
    ).await;
    assert_eq!(control, ferrum_edge::_test_support::DiscoveryApplyControlForTest::Continue);
    let gateway = passive_reload_gateway(state.clone()).await;
    let client = reqwest::Client::builder().no_proxy()
        .timeout(std::time::Duration::from_secs(5)).build().unwrap();
    let ejected = |proxy: &str| state.health_checker.passive_health
    ...
    state.health_checker.report_response(
        "ferrum", "partial", "reload-discovered", &bad_target, 503, false, Some(&passive),
    );
    assert_eq!(state.health_checker.passive_recent_failure_len_for_test("ferrum", "partial", &bad_key), 1);
    let bad_hits = bad.received_requests().await.unwrap().len();
    apply_passive_reload(&state, incremental).await;
    for proxy in ["discovered", "static"] {
        assert!(ejected(proxy), "reload re-admitted {proxy}'s ejected endpoint");
        for _ in 0..8 {
            let response = client.get(format!("{}/{proxy}/check", gateway.url)).send().await.unwrap();
    ...
    for remaining in snapshots {
        let removed = if remaining.is_empty() { &good_target } else { &bad_target };
        let removed_key = if remaining.is_empty() { &good_key } else { &bad_key };
        let control = ferrum_edge::_test_support::apply_service_discovery_snapshot_for_test(
            "ferrum", "reload-discovered", "scripted", publish(remaining), &mut discovery,
            &state.load_balancer_cache, &request_epoch, &[], LoadBalancerAlgorithm::RoundRobin,
            &None, &cancel_rx, &None, &state.dns_cache, &state.health_checker,
            &state.circuit_breaker_cache,
        ).await;
        ...
        assert_eq!(state.health_checker.passive_recent_failure_len_for_test("ferrum", "partial", removed_key), 0);
        state.health_checker.report_response(
            "ferrum", "partial", "reload-discovered", removed, 503, false, Some(&passive),
        );
        assert_eq!(state.health_checker.passive_recent_failure_len_for_test("ferrum", "partial", removed_key), 1);
        apply_passive_reload(&state, incremental).await;
        assert_eq!(state.health_checker.passive_recent_failure_len_for_test("ferrum", "partial", removed_key), 0);
        assert!(ejected("static"), "discovery retirement crossed upstream ownership");
    }
    drop(gateway);
