...
async fn test_admin_state_read_only_field() {
    ...
    assert!(admin_state_read_only.read_only, "Admin state should be read-only");
    
    // Test read-write state
    let admin_state_read_write = create_test_admin_state(&config, false);
    assert!(!admin_state_read_write.read_only, "Admin state should not be read-only");
}
...
async fn test_admin_api_integration() {
    ...
    assert!(!admin_state.read_only, "Default admin state should not be read-only");
...
async fn test_admin_read_only_mode_configuration() {
    ...
    let admin_state_read_only = create_test_admin_state(&config, true);
    assert!(admin_state_read_only.read_only, "Read-only mode should be enabled");
    
    // Test read-write mode configuration
    let admin_state_read_write = create_test_admin_state(&config, false);
    assert!(!admin_state_read_write.read_only, "Read-write mode should not be read-only");
}
