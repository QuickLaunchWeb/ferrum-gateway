...
#[tokio::test]
async fn empty_allow_list_rejects_all_in_allow_mode() {
    // An empty config in allow_first mode has no allow and no deny,
    // so both lists are empty. With no allow list configured, the code
    // skips the allow check and falls through to deny (also empty) -> Continue.
    // But if we explicitly pass an empty allow array, it's still empty so
    // the is_empty() check means "not configured" -> falls through.
    let plugin = IpRestriction::new(&json!({
        "allow": []
    }));

    // Empty allow list means is_empty() == true, so allow check is skipped.
    // Deny list is also empty, so the request passes through.
    // This tests that an explicitly empty allow list behaves as "not configured".
    let mut ctx = create_context_with_ip("1.2.3.4");
    let result = plugin.on_request_received(&mut ctx).await;
    plugin_utils::assert_continue(result);
}

#[tokio::test]
async fn empty_deny_list_allows_all() {
    let plugin = IpRestriction::new(&json!({
        "deny": []
    }));
...
#[tokio::test]
async fn empty_deny_list_deny_first_mode_allows_all() {
    let plugin = IpRestriction::new(&json!({
        "deny": [],
        "mode": "deny_first"
    }));
...
#[tokio::test]
async fn default_config_allows_all_traffic() {
    let plugin = IpRestriction::new(&json!({}));
...
#[tokio::test]
async fn default_config_allows_any_ip() {
    let plugin = IpRestriction::new(&json!({}));

    for ip in &["127.0.0.1", "10.0.0.1", "255.255.255.255"] {
        let mut ctx = create_context_with_ip(ip);
        let result = plugin.on_request_received(&mut ctx).await;
        plugin_utils::assert_continue(result);
    }
}
