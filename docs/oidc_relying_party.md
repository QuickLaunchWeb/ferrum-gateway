...
- When `session.cookie_name` is omitted, Ferrum derives a context-specific name prefixed with `__Host-` when `session.domain` is unset and `session.path` is `/` (the default), or `__Secure-` otherwise. These generated cookies always carry `Secure`. An explicit `session.cookie_name` opts out of automatic naming and retains the configured session attributes. Changing from the former unprefixed default requires users to log in again.
- `session.store` supports only `cookie`; Redis-backed server-side sessions are rejected until implemented.
- Browser authorization-code flows are sealed into a distinct short-lived, host-only HttpOnly/SameSite correlation cookie with AES-256-GCM under the same `session.encryption_secret` (and optional `encryption_secret_previous`) as sessions, but with a distinct associated-data label so session cookies cannot be reused as pending-flow state. The OAuth `state` query parameter stays an opaque handle that selects the cookie name; PKCE verifier, nonce, original redirect, expiry, and context binding live only in the sealed cookie — never in URLs, logs, or unsealed cookie values. `session.domain` applies only to the durable session cookie and is never applied to this pending-flow cookie. Correlation cookie names always begin with `__Host-` and always carry `Secure`, `Path=/`, and no `Domain`, including when `session.secure` is false. The AEAD seal and OAuth `state` equality check bind the flow; callback-path scoping is not required. Use HTTPS for development unless the browser supports Secure prefixed cookies on your localhost/loopback origin. Sealed pending-flow cookies honor `session.max_cookie_bytes`; oversized, expired, wrong-context, wrong-browser, tampered, or otherwise invalid state fails closed.
...
Requests to `providers[].logout_path` expire the browser's local session cookie. With an authentic, context-bound session cookie and `behavior.rp_initiated_logout` enabled (the default), Ferrum redirects to the provider's `end_session_endpoint` with the stored ID token as `id_token_hint`, the configured `client_id`, and `post_logout_redirect_uri` when configured. Without a session cookie, or with an invalid cookie, logout returns the local logged-out page and does not contact the provider. An expired ID token may still be sent as a logout hint; it does not authorize a proxy request.
